...
async def test_rf_engine_ghost_and_buy_branches(monkeypatch):
    ...
    rf_mod._last_buy_attempt["R_100S"] = {"direction": "PUT", "timestamp": rf_mod.datetime.now()}
    assert await eng._check_for_ghost_contract("R_100S", "CALL") is None

    rf_mod._last_buy_attempt["R_100S"] = {"direction": "CALL", "timestamp": rf_mod.datetime.now()}
    ...
    eng.ensure_connected = AsyncMock(return_value=True)
    assert await eng.buy_rise_fall("R_100S", "SIDEWAYS", 1.0) is None

    # no connection branch in buy
    eng.ensure_connected = AsyncMock(return_value=False)
    assert await eng.buy_rise_fall("R_100S", "CALL", 1.0) is None
    ...
    ok = await eng.buy_rise_fall("R_100S", "CALL", 1.0)
