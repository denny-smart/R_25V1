...
@pytest.fixture
def mock_components():
    with patch("app.bot.runner.DataFetcher", new_callable=MagicMock) as mock_df, patch(
        "app.bot.runner.TradeEngine", new_callable=MagicMock
    ) as mock_te, patch(
        "app.bot.runner.UserTradesService", new_callable=MagicMock
    ) as mock_uts, patch(
        "app.bot.runner.event_manager", new_callable=AsyncMock
    ) as mock_em, patch(
        "app.bot.runner.telegram_bridge", new_callable=MagicMock
    ) as mock_tb:
...
async def test_bot_runner_analyze_symbol_no_data(mock_components):
    ...
    runner.data_fetcher.fetch_all_timeframes = AsyncMock(
...
@pytest.mark.asyncio
async def test_bot_runner_analyze_symbol_insufficient_data(
    mock_components, sample_ohlc_data
):
    runner = BotRunner(account_id="test_user")
    runner.data_fetcher = mock_components["df"].return_value
    # Mock successful fetch but empty/insufficient data
    runner.data_fetcher.fetch_all_timeframes = AsyncMock(
        return_value={"1m": sample_ohlc_data(n=1)}
    )
...
async def test_bot_runner_scan_cycle_executes_trade(mock_components, sample_ohlc_data):
    ...
    runner.data_fetcher.fetch_all_timeframes = AsyncMock(
        return_value={
            "1m": sample_ohlc_data(n=100),
            "5m": sample_ohlc_data(n=20),
            "1h": sample_ohlc_data(n=10),
            "4h": sample_ohlc_data(n=5),
            "1d": sample_ohlc_data(n=5),
            "1w": sample_ohlc_data(n=5),
        }
    )
    ...
    runner.strategy = MagicMock()
    runner.strategy.get_required_timeframes.return_value = [
        "1m",
        "5m",
        "1h",
        "4h",
...
async def test_bot_runner_monitor_active_trade_conservative_stays_open(mock_components):
    ...
    runner.trade_engine = mock_components["te"].return_value
    runner.trade_engine.get_trade_status = AsyncMock(
        return_value={"is_sold": False, "profit": 2.0}
    )
...
async def test_bot_runner_monitor_active_trade_scalping_stagnation(mock_components):
    ...
    runner.trade_engine = mock_components["te"].return_value
    runner.trade_engine.get_trade_status = AsyncMock(
        return_value={"is_sold": False, "profit": -0.5}
    )
    runner.trade_engine.close_trade = AsyncMock(
        return_value={"status": "closed", "profit": -0.5}
    )
...
@pytest.mark.asyncio
async def test_bot_runner_monitor_active_trade_scalping_trailing_profit(
    mock_components,
):
    # Mock ScalpingRiskManager to pass isinstance check
    from scalping_risk_manager import ScalpingRiskManager
    ...
    runner.trade_engine = mock_components["te"].return_value
    runner.trade_engine.get_trade_status = AsyncMock(
        return_value={"is_sold": False, "profit": 5.0}
    )
    runner.trade_engine.close_trade = AsyncMock(
        return_value={"status": "closed", "profit": 5.0, "contract_id": "123"}
    )
...
@pytest.mark.asyncio
async def test_bot_runner_manual_sync_risk_close_persists_exit_price_and_duration(
    mock_components,
):
    runner = BotRunner(account_id="test_user")
    runner.strategy = MagicMock()
    ...
    event_types = [
        call.args[0]["type"] for call in mock_components["em"].broadcast.await_args_list
    ]
    assert "trade_closed" in event_types
    assert "bot_status" in event_types


@pytest.mark.asyncio
async def test_bot_runner_manual_sync_external_close_persists_exit_price_and_duration(
    mock_components,
):
    runner = BotRunner(account_id="test_user")
    runner.strategy = MagicMock()
...
@pytest.mark.asyncio
async def test_bot_runner_scalping_manual_sync_external_close_persists_multiplier(
    mock_components,
):
    from scalping_risk_manager import ScalpingRiskManager
...
@pytest.mark.asyncio
async def test_bot_runner_monitor_active_trade_scalping_trailing_activation(
    mock_components,
):
...
@pytest.mark.asyncio
async def test_runner_startup_reconcile_handles_manual_and_system_entries(
    mock_components,
):
    runner = BotRunner(account_id="test_user")
    runner.strategy = MagicMock()
