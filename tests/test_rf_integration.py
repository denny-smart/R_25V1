...
async def test_trade_lock_prevents_concurrent_execution():
    ...
    acquired = await rm.acquire_trade_lock("R_25", "contract_001")
...
async def test_db_failure_halts_and_keeps_lock():
    ...
    await rm.acquire_trade_lock("R_25", "contract_fail_test")
    ...
    acquired = await rm.acquire_trade_lock("R_25", "should_fail")
...
async def test_full_lifecycle_step_logging(caplog):
    ...
    with caplog.at_level(logging.INFO, logger="risefallbot.risk"):
        # Step 1: Acquire lock
        await rm.acquire_trade_lock("R_25", "contract_log_test")
...
async def test_mutex_blocks_concurrent_acquire():
    ...
    await rm.acquire_trade_lock("R_25", "first_trade")
